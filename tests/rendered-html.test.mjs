...
test("renders the 5 by 8 Tetris player", async () => {
  const response = await render("/tetris");
  assert.equal(response.status, 200);
  const html = await response.text();
  assert.match(html, /TAKOYAKI TETRIS/);
  assert.match(html, /5 × 8 · 40 CELLS/);
  assert.match(html, /HARD DROP/);
  assert.match(html, /LIVE · 2 PLATES/);
});
...
test("keeps cooking and Tetris projection geometry explicit", async () => {
  ...
  assert.match(tetrisSource, /TETRIS_COLS = 5/);
  assert.match(tetrisSource, /TETRIS_ROWS = 8/);
  assert.match(tetrisSource, /TETRIS_PLATE_ROWS = 4/);
  assert.match(projectorSource, /PlateOutput id="A"/);
  assert.match(projectorSource, /PlateOutput id="B"/);
});
